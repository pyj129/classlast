...
st.title("🎨 What is Most Similar: Draw the Cosine Function!")

st.markdown("좌표평면 위에 코사인 함수를 그려보세요.")
...
def create_coordinate_grid_svg(width, height):
    ...
    # Y축 (왼쪽 수직선, x = 0에서 코사인 시작을 보기 쉽게 하기 위함)
    x_axis_pos = padding
    svg_lines.append(
    ...
    for y_val, y_label in y_labels:
        ...
        # 눈금선
        svg_lines.append(
            f'<line x1="{x_axis_pos}" y1="{y_pos}" x2="{x_axis_pos + 8}" y2="{y_pos}" '
            f'stroke="black" stroke-width="1.5"/>'
        )
        # 라벨 (Y축이 왼쪽 끝에 있으므로 살짝 왼쪽으로 배치)
        svg_lines.append(
            f'<text x="{x_axis_pos - 10}" y="{y_pos + 4}" text-anchor="end" font-size="11" font-weight="bold">'
            f'{y_label}</text>'
        )
...
# ============================================================================
st.subheader("✏️ Draw the Cosine Function")
...
if canvas_result.image_data is not None:
    ...
    try:
        ...
        if len(black_pixels[0]) > 20:
            ...
            coords = coords[np.argsort(coords[:, 0])][unique_x_indices]
            ...
            if len(coords) > 10:
                ...
                # 캔버스 픽셀 좌표 → 수학 좌표로 변환
                # 여백(padding=40)을 고려한 정확한 매핑을 위해 내부 plot_width 기준으로 정규화하는 것이 좋으나, 
                # 기존 코드의 방식(전체 너비 대비 비율)을 유지하되 코사인 계산에 맞춰 적용합니다.
                ...
                if len(x_math) > 10:
                    ...
                    try:
                        ...
                        y_full = np.cos(x_full)  # 코사인 함수로 변경!

                        ax.plot(x_full, y_full, 'b-', linewidth=3, label='Actual cos(x)', alpha=0.7)
                        ...
                        # 유사도 계산 (cos 기준)
                        mse = np.mean((y_interp - np.cos(x_interp)) ** 2)
